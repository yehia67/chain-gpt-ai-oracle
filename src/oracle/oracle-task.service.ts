...
export class OracleTaskService {
  ...
  async getTask(taskId: string): Promise<OracleTaskDetailsResponse> {
    ...
    return {
      ...
      txHash: task.txHash,
      x402ConditionHash: task.x402ConditionHash,
      x402ConditionSignature: task.x402ConditionSignature,
      x402SettlementRef: task.x402SettlementRef,
      x402ExecutionMode: task.x402ExecutionMode,
